...
const Paginate = ({ pagina, setPagina, maximo }) => {
    ...
    const onKeyDown = (event) => {
        if (event.keyCode == 13) {
            ...
            if (
                parseInt(event.target.value < 1) || parseInt(event.target.value) > Math.ceil(maximo) || isNaN(parseInt(event.target.value))
            ) {
                setPagina(1);
                setInput(1);
            }
    ...
    return (
        <div className={style.container}>
            <button disabled={pagina === 1 || pagina < 1} className={style.ButtonPage} onClick={previusPage}>
